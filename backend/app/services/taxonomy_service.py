...
def update_category(db: Session, category_id: int, payload: CategoryUpdate, *, user_id: int) -> Category:
    ...
    if "parent_id" in updates:
        _validate_parent_category(db, updates["parent_id"], user_id=user_id, current_id=category.id)
...
def delete_category(db: Session, category_id: int, *, user_id: int) -> DeleteResponse:
    category = get_category(db, category_id, user_id=user_id)
    db.delete(category)
...
def get_tags_by_names(db: Session, tag_names: list[str], *, user_id: int) -> list[Tag]:
    if not tag_names:
        return []
...
def get_or_create_tags(db: Session, tag_names: Iterable[str], *, user_id: int) -> list[Tag]:
...
def create_tag(db: Session, payload: TagCreate, *, user_id: int) -> Tag:
    tag = Tag(
        user_id=user_id,
        name=normalize_required_text(payload.name, field_name="name"),
        created_at=utc_now(),
...
def update_tag(db: Session, tag_id: int, payload: TagUpdate, *, user_id: int) -> Tag:
...
def delete_tag(db: Session, tag_id: int, *, user_id: int) -> DeleteResponse:
    tag = get_tag(db, tag_id, user_id=user_id)
    db.delete(tag)
    db.commit()
